...
VCORE_INGEST_SECRET=$(grep ^VCORE_INGEST_SECRET .env | cut -d '=' -f2-)
BATHQUBE_FROM_EMAIL=$(grep ^BATHQUBE_FROM_EMAIL .env | cut -d '=' -f2-)
FB_PAGE_ACCESS_TOKEN=$(grep ^FB_PAGE_ACCESS_TOKEN .env | cut -d '=' -f2-)
FB_PAGE_ID=$(grep ^FB_PAGE_ID .env | cut -d '=' -f2-)
FB_APP_SECRET=$(grep ^FB_APP_SECRET .env | cut -d '=' -f2-)
FB_WEBHOOK_VERIFY_TOKEN=$(grep ^FB_WEBHOOK_VERIFY_TOKEN .env | cut -d '=' -f2-)
RESEND_API_KEY=$(grep ^RESEND_API_KEY .env | cut -d '=' -f2-)
RESEND_SENDER_EMAIL=$(grep ^RESEND_SENDER_EMAIL .env | cut -d '=' -f2-)
GLASSY_WEBHOOK_SECRET=$(grep ^GLASSY_WEBHOOK_SECRET .env | cut -d '=' -f2-)
...
if [ -z "$LAMBDA_EXISTS" ]; then
    ...
    aws lambda create-function \
        ...
        --memory-size 512 \
        --region $AWS_REGION \
        --environment "Variables={DATABASE_URL=$DATABASE_URL,SECRET_KEY=$SECRET_KEY,ENVIRONMENT=production,WORDPRESS_URL=$WORDPRESS_URL,WORDPRESS_API_USER=$WORDPRESS_API_USER,WORDPRESS_API_PASSWORD=$WORDPRESS_API_PASSWORD,WORDPRESS_SYNC_ENABLED=$WORDPRESS_SYNC_ENABLED,CRON_SECRET=$CRON_SECRET,APP_URL=$APP_URL,SES_SENDER_EMAIL=$SES_SENDER_EMAIL,AWS_BUCKET_NAME=$AWS_BUCKET_NAME,VCORE_INGEST_SECRET=$VCORE_INGEST_SECRET,BATHQUBE_FROM_EMAIL=$BATHQUBE_FROM_EMAIL,FB_PAGE_ACCESS_TOKEN=$FB_PAGE_ACCESS_TOKEN,FB_PAGE_ID=$FB_PAGE_ID,FB_APP_SECRET=$FB_APP_SECRET,FB_WEBHOOK_VERIFY_TOKEN=$FB_WEBHOOK_VERIFY_TOKEN,RESEND_API_KEY=$RESEND_API_KEY,RESEND_SENDER_EMAIL=$RESEND_SENDER_EMAIL,GLASSY_WEBHOOK_SECRET=$GLASSY_WEBHOOK_SECRET}" \
